...
| Q21 | Does DayZ announce the server a player is on to Steam (`FriendGameInfo_t.m_unGameIP/m_usGamePort`)? The Friends tab (D-092) can only offer Join when it does; the one DayZ friend seen during the live check was idle and reported `0.0.0.0:0` | Friends tab usefulness | Run `cargo test --test live_friends -- --ignored --nocapture` while a friend is playing on a server; if the address stays empty, look at DayZ's Steam rich presence (`connect` key via `GetFriendRichPresence`) as an alternative |
