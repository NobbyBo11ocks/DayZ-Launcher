...
| Q19 | Default per-user install dir `%LOCALAPPDATA%\DayZ Launcher` collides with the official launcher's data folder (D-064). Options: (a) custom NSIS template (copy of S-56) with `$LOCALAPPDATA\Programs\${PRODUCTNAME}`, the Windows convention for per-user apps; (b) a different `productName`; (c) `installMode: perMachine` (Program Files, needs UAC) | next release | Decision; (a) recommended, keeps the name and needs no elevation |
