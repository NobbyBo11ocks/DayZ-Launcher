...
| Host process (`dayz-launcher.exe`) private bytes, idle | < 90 MB with list loaded and Steamworks initialised | M0 5.9 MB; M4 65–79 MB; M7 70 MB flat over 7 min (82 during refresh); **v0.1.5 70.3 MB at 7 min** with the GeoIP table read in place (D-082; 81–94 MB with the copied table). `steam_api64.dll` loads `steamclient64.dll` and `gameoverlayrenderer64.dll` in-process; releasing Steamworks when idle does not lower this (D-057) | `Get-Process dayz-launcher \| select PrivateMemorySize64`; Diagnostics → Performance |
