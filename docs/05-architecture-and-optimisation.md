...
| Cold start to first painted list | < 1.0 s | v0.1.4+perf 795 ms (process start → first frame with rows, 33 480 cached rows, fresh process after install; a warm second instance showed 314 ms); **v0.1.6 814 ms** (27 249 cached rows, first start after the installer) | Diagnostics → Performance (`perf_first_paint`, D-078) |
