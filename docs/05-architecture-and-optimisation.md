...
| Populated-server refresh | first rows in seconds, whole pass < 60 s | Criterion replaced at M2 acceptance: "20 000 servers in 15 s" is neither reachable nor needed, because throughput is bounded by the consumer NAT flow table and Steamworks already supplies the list with pings (D-037). Measured: M3 ≈ 38 s; **v0.1.19: 2 379 servers from Steam in 36 s, then 2 365 verified, 10 fake, 4 offline**; a manual "Full" pass adds the empty servers over minutes and reached 19 088 rows | the Servers header line; app log |
