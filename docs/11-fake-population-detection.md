...
| T2 query-port proxy rewriting INFO *and* PLAYER | anatolykopyl, Go, last commit 2026-01; durations `random() × 10 000` per query | R0 with zero real players; otherwise **R11 only** |
...
| T9 caching DDoS proxies (up to 10 s stale) | gnif SteamQueryProxy, 0x280 | not a fake; R11's ±15 s slack absorbs it |
