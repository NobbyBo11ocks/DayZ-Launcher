...
A full per-map refresh (D-046) put the scale beyond doubt: **27 037 of 33 605 responding servers are inflated**, i.e. Steam's list holds roughly 6 500 genuine DayZ servers (≈ 3 000 populated, ≈ 3 900 empty) and about 27 000 fakes, concentrated on chernarusplus, enoch and namalsk (each capped at 10 000 with 77–90 % fakes). Because of that, the automatic refresh only asks Steam for `hasplayers` servers; empty servers are a manual "Full refresh".
