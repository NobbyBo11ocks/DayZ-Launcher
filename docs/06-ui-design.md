...
- The rail is 240 px wide (180 px until D-249 widened it for the filters). It holds the sections, the Servers filters under them while that page is open (§4), and the update item at its foot (D-216). Section icons are in the accent and their labels in the default text colour (D-253; D-250 had the labels in the accent as well); the open section has the raised surface, the accent bar and a heavier weight, and the update item an accent tint.
