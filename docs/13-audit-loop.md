...
| 8 | **Security and privacy** (carried from row 5: `junction = "2.0.0"` builds with its default `unstable_admin` feature, which enables SeBackup/SeRestore on the process token when an elevated process is refused a reparse point; `default-features = false` would ask for SeCreateSymbolicLink instead) | IN PROGRESS since 2026-09-26T12:46Z | |
