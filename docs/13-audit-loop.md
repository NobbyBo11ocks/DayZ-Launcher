...
| 8 | **Security and privacy** (carried from row 5: `junction = "2.0.0"` builds with its default `unstable_admin` feature, which enables SeBackup/SeRestore on the process token when an elevated process is refused a reparse point; `default-features = false` would ask for SeCreateSymbolicLink instead) | todo | |
...
- From D-279: (a) check for updates again while the launcher stays open (on window focus, once a day has passed), so "Update Available" can appear mid-session instead of only at the next start; (b) when Windows refuses to start the downloaded setup (antivirus, for instance), the launcher has already hidden its window and stopped Steam: show the window again with "Could not start the {version} setup ({reason}). Restart the launcher to use it again."; (c) ask before "Install and restart" while DayZ is running or a mod download is in progress.
