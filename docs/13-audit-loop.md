...
- From D-276: (a) the Mods page's Servers column and its "On no server" chip read 0 for every mod until the server list's mod index has loaded, and for good if that load fails — show "—" and disable the chip until the index is in, and drop "Populated" from the column's tooltip (a server counts if its list was read, whatever its players now); (b) a failed download names the mod it failed on, and an unsubscribe replaced by a newer one ("superseded by a newer request") shows as a notice, not an error.
