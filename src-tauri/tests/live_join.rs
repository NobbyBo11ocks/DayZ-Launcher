...
fn live_join() {
    let mod_id: u64 = std::env::var("DAYZ_TEST_MOD").ok().and_then(|v| v.parse().ok()).unwrap_or(1_819_514_788);
    ...
    loop {
        match rx.try_recv() {
            ...
            Ok(SteamEvent::Status(s)) if s.error.is_some() => panic!("steam init failed: {:?}", s.error),
            _ => {}
        }
        assert!(t0.elapsed() < Duration::from_secs(15), "steam did not initialise");
    ...
    println!("mod {}: {:?} {} bytes, updated {}", mod_id, details[0].title, details[0].file_size, details[0].time_updated);
    ...
    let done = loop {
        match rx.try_recv() {
            Ok(SteamEvent::SyncProgress(p)) => {
                ...
                if it.state != last_state {
                    println!("  {:>6.1}s {} {}/{} bytes", t1.elapsed().as_secs_f64(), it.state, it.downloaded, it.total);
        ...
        assert!(t1.elapsed() < Duration::from_secs(600), "sync did not finish in 10 min");
        std::thread::sleep(Duration::from_millis(100));
    };
    println!("sync done ok={} in {} ms: {:?}", done.ok, done.elapsed_ms, done.items[0]);
    assert!(done.ok, "sync failed: {:?}", done.error);
    let folder = PathBuf::from(done.items[0].folder.clone().expect("install folder"));
    assert!(folder.is_dir(), "installed folder exists: {}", folder.display());
    ...
    let ws = workshop::read(&game.library.path).unwrap().expect("workshop manifest");
    let item = ws.items.iter().find(|i| i.id == mod_id).expect("mod in appworkshop acf");
    println!("acf: meta_name {:?} mod_name {:?} folder {:?}", item.meta_name, item.mod_name, item.folder);
    let before = workshop::junctions(&game.workshop_dir()).len();
    let links = ensure_junctions(&game.folder, &[(mod_id, folder.clone(), item.meta_name.clone())]).unwrap();
    println!("junction: {} (created: {})", links[0].junction.display(), links[0].created);
    assert!(junction::exists(&links[0].junction).unwrap());
    let after = workshop::junctions(&game.workshop_dir()).len();
    assert!(after >= before, "no junction was removed ({before} -> {after})");
    ...
    let (ip, port) = if let Ok(s) = std::env::var("DAYZ_TEST_SERVER") {
        ...
        let db = dirs_local().join("com.dayzlauncher.desktop").join("cache.db");
        ...
        let pick = rows
            ...
            .filter(|r| r.verdict.as_deref() == Some("verified") && r.verified_players.unwrap_or(0) > 0)
            .filter(|r| local.as_deref().is_none_or(|v| v == r.version))
            .min_by_key(|r| r.ping_ms)
            .expect("a verified vanilla server in the cache");
        println!("server: {} ({}:{}) {} players, {} ms, v{}", pick.name, pick.ip, pick.game_port, pick.verified_players.unwrap_or(0), pick.ping_ms, pick.version);
    ...
    while t2.elapsed() < Duration::from_secs(30) {
        if let Ok(Some(status)) = child.try_wait() {
            println!("exited early after {:?} with {:?}", t2.elapsed(), status.code());
            alive = false;
            break;
        }
    ...
    if alive {
        ...
        let _ = std::process::Command::new("taskkill").args(["/PID", &launched.pid.to_string(), "/T", "/F"]).status();
        let _ = std::process::Command::new("taskkill").args(["/IM", "DayZ_x64.exe", "/F"]).status();
        let _ = child.wait();
        println!("process was alive after 30 s; killed");
    }
    assert!(alive, "DayZ_BE.exe exited within 30 s (BattlEye/game refused to start)");
}
